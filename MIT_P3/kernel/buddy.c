...
#define ROUNDUP(n,sz) (((((n)-1)/(sz))+1)*(sz))  // Round up n to the next multiple of sz
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if(bit_isset(bd_sizes[k].alloc, blk_index(k, p)/2)) //buddy is free
      bit_clear(bd_sizes[k].alloc, blk_index(k, p)/2);
    else{
      bit_set(bd_sizes[k].alloc, blk_index(k, p)/2); //buddy is still allocated, break the loop
...
int                                                         //int
blk_index_next(int k, char *p) {                            //blk_index(int k, char *p) {
  int n = (p - (char *) bd_base) / BLK_SIZE(k);             //  int n = p - (char *) bd_base;
  if((p - (char*) bd_base) % BLK_SIZE(k) != 0)              //  return n / (BLK_SIZE(k));
      n++;                                                  //}
  return n ;
}
...
int
bd_initfree_pair(int k, int bi) {//bd_initfree_pair(int k, int bi, int lf) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, bi/2)){
    free = BLK_SIZE(k);
...
int
bd_initfree(void *bd_left, void *bd_right) {//int free = bd_initfree(p, bd_end);
  int free = 0;

  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    int left = blk_index(k, bd_left);
...
bd_mark_data_structures(char *p) {
  ...
  printf("bd: %d %p meta bytes for managing %d %p bytes of memory start from: %p, end at: %p\n", 
    meta, meta, BLK_SIZE(MAXSIZE), BLK_SIZE(MAXSIZE), bd_base, p);
  bd_mark(bd_base, p);
  return meta;
}
...
bd_init(void *base, void *end) {
  ...
  printf("bd: memory sz is %d %p bytes; start at: %p; end at: %p; allocate an size array of length %d\n",
         (char*) end - p,(char*) end - p, p, end, nsizes);

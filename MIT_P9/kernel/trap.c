...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    syscall();
  ...
  } else if(r_scause()==0xf||r_scause()==0xd){ //page fault
      uint64 va = r_stval();
      //printf("va: %p\n",va);
      struct proc *p = myproc();
      struct vma *vma;
      ...
      for(vma = p->vma; vma < &p->vma[NBVMA]; vma++){
        ...
        if(va >= vma->address && va <= (vma->address + vma->length)){
          found = 1;
          //printf("vma->address: %p\n",vma->address);
          //printf("vma->length: %p\n",vma->length);
          break;
      ...
      if(!found){
        ...
        if(mem == 0){
          ...
          memset(mem, 0, PGSIZE);
          ilock(vma->f->ip);
          readi(vma->f->ip, 0, (uint64)mem, a - vma->address + vma->offset, PGSIZE);
          iunlock(vma->f->ip);
          int perm=0;
          if(vma->prot == PROT_READ)
            ...
            perm = PTE_W|PTE_U;
          else if(vma->prot == (PROT_READ|PROT_WRITE))
            perm = PTE_W|PTE_R|PTE_U;
          //printf("mappages1\n");
          //printf("a: %p\n",a);

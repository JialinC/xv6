...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(WP != NULL){
        acquire(&WP->lock);
        WP->state = RUNNING;
        WP->start_tick = ticks; //p2 the starting tick of this scheduling round
        c->proc = WP;
...
yield(void)
{
  ...
  acquire(&p->lock);
  p->state = RUNNABLE;
  p->total_tick+=(ticks - p->start_tick); //p2, calculate the total tick passed since first start up
  sched();
  release(&p->lock);
  release(&sched_lock); //for access the process table
}

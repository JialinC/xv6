...
e1000_recv(void)
{
  //printf("check point\n");
  int have = 1;
  //printf("regs[E1000_RDT] %d\n",regs[E1000_RDT]);
  while(have){
    int next = (regs[E1000_RDT]+1)%RX_RING_SIZE;
    //printf("next in recv: %d\n",next);
    if(rx_ring[next].status & E1000_RXD_STAT_DD){
      //printf("m.length: %p\n",((struct mbuf*)rx_ring[next].addr)->len);
      mbufput(rx_mbufs[next],rx_ring[next].length);
      net_rx(rx_mbufs[next]);
      //printf("print out buffer\n");
      //printf("buffer length: %p\n",rx_mbufs[next]->len);
      //printf("buffer next: %p\n",rx_mbufs[next]->next);
      //printf("buffer head: %p\n",rx_mbufs[next]->head);
      //printf("m.length: %p\n",((struct mbuf*)rx_ring[next].addr)->len);
      //printf("m.length: %p\n",rx_mbufs[next]->len);
      //printf("rx_ring[next].length: %p\n",rx_ring[next].length);
      //printf("while status: %p\n",rx_ring[next].status);
